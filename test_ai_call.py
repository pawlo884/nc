            ...
            if 'ai_request_prepared' in expr:
                ...
                try:
                    ...
                    if isinstance(result, dict):
                        print(f"ai_request_prepared: {result.get('ai_request_prepared')}")
                        print(f"original_text obecny: {bool(result.get('original_text'))}")
                        if result.get('ai_request_prepared'):
                            ...
                            if original_text:
                                print(f"Wywołuję edit_description_with_ai z tekstem długości {len(original_text)}...")
                                edited = edit_description_with_ai(
                                    original_description=original_text,
                                    product_name=result.get('product_name', ''),
                                    brand_name=result.get('brand_name', '')
                                )
                                print(f"Wynik edycji: długość {len(edited)} znaków")
                                print(f"Pierwsze 200 znaków: {edited[:200]}...")
