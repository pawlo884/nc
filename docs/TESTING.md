...
### Pokrycie matterhorn1 (211 testów, po #218)
...
| `admin.py`                           | 40%       | 903 stmt — `mpd_create`, `assign_mapping`, `bulk_map_to_mpd`, `auto_map_variants` pokryte (był 30%); `bulk_create_mpd_action` (głównie render szablonu) świadomie pominięty jako niski priorytet                                                                                                                                          |
| `views.py`                           | 32%       | 394 stmt — legacy, równoległy do `views_secure.py` (hardened), oba routowane w `urls.py`. Chroniony przez `core.legacy_auth` od PR #110. Testy ujawniły [#221](https://github.com/pawlo884/nc/issues/221): 4 miejsca szukały produktu po nieistniejącym polu `product_id` zamiast `product_uid` — zawsze 500/"nie znaleziono". Naprawione |
| `defs_db.py` / `bestsellers_data.py` | 27% / 14% |                                                                                                                                                                                                                                                                                                                                           |
| **TOTAL**                            | **55%**   | próg CI: 53% (ratchet, nie cel)                                                                                                                                                                                                                                                                                                           |

**Cel „linie ≥ 80%" jest poza zakresem tego planu** — wymaga pokrycia reszty
`admin.py`. Logika krytyczna (import, saga engine, stock tracking,
serializery, modele) jest w przedziale 64–93%.

**Podnoszenie progu / dalej:** dopisać ścieżki innych apek do `--cov` w
`check-branch.yml` gdy dostaną testy.
