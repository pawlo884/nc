# Konfiguracja systemu gateway.xml z węzłem changes
...
3. **W gateway.xml dodano węzeł `full` z podwęzłem `changes`** zawierającym węzły `change` z linkami do wszystkich plików full_change
...
# Odśwież gateway.xml (zaktualizuje węzeł full z changes)
...
### Węzeł changes nie pojawia się w gateway.xml

```python
# Sprawdź czy są pliki full_change w bazie
from MPD.models import FullChangeFile
files = FullChangeFile.objects.all()
print(f"Liczba plików w bazie: {files.count()}")

# Jeśli są pliki, odśwież gateway.xml
if files.exists():
    from MPD.export_to_xml import export_gateway_xml
    export_gateway_xml()
```
...
with open('MPD_test/xml/gateway.xml', 'r') as f:
    ...
    if '<full' in content and '<changes>' in content:
        print("✅ Węzeł full z changes został dodany")
    else:
        print("❌ Węzeł full z changes nie został znaleziony")
```
...
```bash
# Uruchom pełny test
python test_new_gateway_system.py
```

## Przykładowy wynik

Po uruchomieniu systemu, gateway.xml będzie wyglądał tak:

```xml
<?xml version="1.0" encoding="utf-8"?>
<provider_description file_format="IOF" version="3.0" generated_by="nc" generated="2025-08-25 11:30:00">
  <meta>
    <!-- ... meta data ... -->
  </meta>
  <full url="https://mojbucket.fra1.digitaloceanspaces.com/MPD_test/xml/full.xml" 
...
1. **Węzeł full** zawiera teraz podwęzeł `changes`
2. **Pliki full_change** mają datę w nazwie
3. **Każdy eksport** tworzy nowy plik (nie nadpisuje)
4. **Wszystkie pliki full_change** są widoczne w węzłu `changes`
5. **Automatyczne odświeżanie** gateway.xml po eksporcie
