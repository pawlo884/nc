...
def check_container_status():
    ...
    try:
        result = subprocess.run(
            ["docker", "ps", "--filter", "name=postgres-ssh-tunnel",
                "--format", "{{.Names}}\t{{.Status}}"],
            capture_output=True,
            text=True,
            timeout=10
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            print(
                f"  Uruchom: {compose_cmd_str()} -f {COMPOSE_FILE} up -d postgres-ssh-tunnel")
            return False
...
def check_port_in_container():
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "exec", "postgres-ssh-tunnel",
                "nc", "-z", "localhost", "5434"],
            capture_output=True,
            text=True,
            timeout=5
        ...
        if result.returncode == 0:
            ...
            print(
                f"  Sprawdź logi: {compose_cmd_str()} -f {COMPOSE_FILE} logs postgres-ssh-tunnel")
            return False
...
def get_web_container_name():
    """Wykrywa nazwę działającego kontenera web (bez hardcodowania)."""
    try:
        result = subprocess.run(
        ...
        # Priorytet: typowa nazwa compose "<project>-web-1"
        for name in names:
            if name.endswith("-web-1"):
                return name

        # Fallback: jakikolwiek kontener zawierający "web"
        for name in names:
...
def check_port_from_web_container():
    ...
    try:
        web_container = get_web_container_name()
        if not web_container:
            print("[WARNING] Nie wykryto uruchomionego kontenera web")
            print("  Pomijam test połączenia web -> tunnel")
            return False

        print(f"[INFO] Wykryty kontener web: {web_container}")

        # Sprawdź czy wykryty kontener web może połączyć się z postgres-ssh-tunnel:5434
        result = subprocess.run(
            ["docker", "exec", web_container, "nc",
                "-z", "postgres-ssh-tunnel", "5434"],
            capture_output=True,
            text=True,
            timeout=5
        )
...
def check_env_variables():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            has_password = "SSH_PASSWORD" in env_output and env_output.count(
                "SSH_PASSWORD=") > 0
...
def compose_cmd():
    """Zwraca dostępny wariant polecenia compose: docker compose lub docker-compose."""
...
def main():
    ...
    if env_ok and container_ok and port_ok:
        ...
        if connection_ok:
            ...
            print(
                f"  {compose_cmd_str()} -f {COMPOSE_FILE} exec web python scripts/check_db_connections.py")
            return 0
        else:
